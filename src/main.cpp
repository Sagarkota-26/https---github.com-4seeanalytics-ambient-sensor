...
static void i2sMicroFFTtask(void *arg)
{
    ...
    while (1)
    {
        ...
        if (currentMillis - previousDataMillis >= 50)
        {
            ...
            else if (state == MODE_MIC)
            {
                ...
                for (int count = 0; count < 24; count++)
                {
                    ...
                    New_data[count] = map(subData, 0, 255, 0, 100);
                }

                for (int n = 0; n < 24; ++n)
                {
                    avg += New_data[n];
                    // Serial.print(String(New_data[n]) + " ");
                }
                // Serial.println("");
                ...
                {
                    cnt = 0;
                    final_avg = final_avg / 100;
                    Serial.println("AVG = " + String(final_avg));
                    final_avg = 0;
                }
                else
                {
...
void microPhoneSetup()
{
    ...
    DisFFTbuff.createSprite(240, 54);
}
...
void MicroPhoneFFT()
{
    ...
    DisFFTbuff.fillRect(0, 0, 240, 54, DisFFTbuff.color565(0x00, 0x00, 0x00));
    ...
    for (int x = 0; x < 24; x++)
    {
        for (int y = 0; y < 9; y++)
        {
            ...
            DisFFTbuff.fillRect(x * 10, 54 - y * 7 - 5, 5, 5, colorRect);
        }
    }
    DisFFTbuff.pushSprite(0, 60);
}
